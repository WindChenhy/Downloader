...
- 新建下载对话框：粘贴链接（**支持多行批量创建**）、自定义文件名（留空自动从链接/响应头获取）、选保存目录、设连接数（1–128）、可选校验算法与期望值
- **下载链接输入框**：最小高度与普通输入框一致，禁止拖拽调高，按内容自动增高（过长时内部滚动）
- **任务详情**：点击任务行查看文件名、链接、下载路径（均可一键复制）、大小、状态、连接数、下载耗时、总耗时、平均速度、校验和、创建/结束时间
- **耗时统计**：
...
| POST | `/api/v1/tasks` | 新建任务，body：`{"url": "...", "saveDir": "...", "connections": 8, "fileName": "自定义文件名", "checksumAlgo": "sha256", "checksumExpected": "..."}`（除 url 外均可省略） |
